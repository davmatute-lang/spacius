...
import com.example.spacius.CalendarAdapter
...
class CalendarActivity : AppCompatActivity() {
    ...
    private fun setupCalendar() {
        ...
        calendarGridView.setOnItemClickListener { _, _, position, _ ->
            ...
            if (selectedDate != null) {
                val dateString = "${selectedDate.get(Calendar.DAY_OF_MONTH)}/" +
    ...
    // Función pública para marcar fecha desde ReservaFragment
    fun marcarFechaDesdeReserva(fecha: String) {
        calendarAdapter.marcarFechaReservada(parseFecha(fecha))
        calendarAdapter.notifyDataSetChanged()
    }

    // Convertir string "dd/MM/yyyy" a Calendar
